...
//  Project23
//
//  Created by Emin Roblack on 4/19/19.
...
class GameScene: SKScene {
    ...
    // Magic numbers
    let enemySpawnLeft: CGFloat = 256
    let enemySpawnMid: CGFloat = 512
    let enemySpawnRight: CGFloat = 768
    
    let enemyPhysicsBodyFrame: CGFloat = 64
	...
	func createEnemy(forceBomb: ForceBomb = .random) {
		...
		// 3
		if randomPosition.x < enemySpawnLeft {
			randomXVelocity = Int.random(in: 8...15)
		} else if randomPosition.x < enemySpawnMid {
			randomXVelocity = Int.random(in: 3...5)
		} else if randomPosition.x < enemySpawnRight {
			randomXVelocity = -Int.random(in: 3...5)
		} else {
			randomXVelocity = -Int.random(in: 8...15)
		}
		...
		// 4
		var randomYVelocity = Int.random(in: velocityLow...velocityHigh)
		...
		// 5
		enemy.physicsBody = SKPhysicsBody(circleOfRadius: enemyPhysicsBodyFrame)
		enemy.physicsBody?.velocity = CGVector(dx: randomXVelocity * enemySpeedBase, dy: randomYVelocity * enemySpeedBase)
	...
	func endGame(triggeredByBomb: Bool) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
            self.gameOverNode = SKSpriteNode(imageNamed: "gameOver")
            self.gameOverNode.zPosition = 1
            self.gameOverNode.position = CGPoint(x: self.frame.width/2, y: self.frame.height/2)
